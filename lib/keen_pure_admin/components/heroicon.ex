defmodule PureAdmin.Components.Heroicon do
  @moduledoc """
  Inline-SVG Heroicons component — ships SVG path data directly, no external
  CSS, no Tailwind plugin, no hex dep.
  ...
  Ships the ~25 most common icons (curated to match pureadmin CLI's
  ...
  defp do_heroicon(%{name: name} = assigns) when is_binary(name) do
    ...
    ~H"""
    <span class={["heroicon-missing", @class]} title={"Unknown heroicon: " <> @missing} aria-label={@aria_label}></span>
    """
  end
end
