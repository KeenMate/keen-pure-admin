  ...
  if (window.FloatingUIDOM) {
    ...
    const update = () => {
      FUI.computePosition(trigger, popconfirm, {
        ...
        // Map Floating UI's physical placement back to our logical class.
        // `.split("-")[0]` handles compound forms like "top-start".
        const finalLogical = physicalToLogical(result.placement.split("-")[0])
...
function physicalToLogical(physical) {
  const rtl = document.documentElement.dir === "rtl"
  if (physical === "left") return rtl ? "end" : "start"
  if (physical === "right") return rtl ? "start" : "end"
  return physical
}
