 ...
 *
 * Fetches theme manifests from /api/themes/manifests and dynamically
 * populates theme, color variant, and mode selectors.
...
export const PureAdminSettings = {
  mounted() {
    ...
    if (this._defaultTheme && !localStorage.getItem("theme")) {
      ...
      console.debug("[PureAdminSettings] No theme in localStorage, applying default:", this._defaultTheme)
    }

    // Fetch manifests, then populate and apply
    this._fetchManifests().then(() => {
      console.debug("[PureAdminSettings] Manifests loaded:", Object.keys(this._themeManifests))
  ...
  _getCurrentManifest() {
    ...
    console.debug("[PureAdminSettings] getCurrentManifest: theme =", theme, ", found =", !!manifest)
    return manifest
  },
  ...
  _updateColorVariantSection(manifest) {
    ...
    console.debug("[PureAdminSettings] updateColorVariantSection: variants =", variants.length, variants.map(v => v.id || "(default)"))
  ...
  _updateModeSection(manifest, variantId) {
    ...
    const modes = this._getModesForVariant(manifest, variantId)
    console.debug("[PureAdminSettings] updateModeSection: variantId =", variantId, ", modes =", modes.length, modes.map(m => m.id))
    if (modes.length <= 1) {
      section.style.display = "none"
      if (modes.length === 1) {
  ...
  _applyThemeCSS(themeId) {
    ...
    const newHref = `/themes/${themeId}/css/${themeId}.css`
