...
// Command palette size presets (rc15). Each maps to a `pa-command-palette--N`
// width modifier that ships in the theme CSS; empty string = the 608px default.
const CMD_PALETTE_SIZES = ["sm", "lg", "xl"]
...
export const PureAdminSettings = {
  ...
  // Notify any code that snapshots CSS vars at draw time (canvas / SVG /
  // hook-mounted charts) that the theme appearance just changed and they
  // should re-read their colours. Mirrors upstream pure-admin's
  // `demo/js/settings-panel.js` contract (kind: "mode" | "variant"), extended
  // here with kind: "theme" tied to the new stylesheet's `load` event for the
  // theme-swap case that upstream doesn't cover.
  _notifyThemeChange(detail) {
  ...
  _applyColorVariant(variant, manifest) {
    ...
    localStorage.setItem("color-variant", variant)
    this._notifyThemeChange({ kind: "variant", variant })
  },
  ...
  _applyThemeMode(mode) {
    ...
    const pattern = manifest?.modeCssClass || "pc-mode-{mode}"
    ...
    this._notifyThemeChange({ kind: "mode", mode })
  },
  ...
  _applyThemeCSS(themeId) {
    ...
    if (themeLink.getAttribute("href") === newHref) return

    // Tie the theme-change event to the new stylesheet's `load` so canvas
    // charts re-sample colours AFTER the new CSS is in effect, not before.
    // One-shot listener: the next swap installs its own. `load`/`error` both
    // fire `notify` so consumers can re-draw even if the CSS 404s (they'll
    // get whatever fallback colours main.css supplies).
    const notify = () => {
      themeLink.removeEventListener("load", notify)
      themeLink.removeEventListener("error", notify)
      this._notifyThemeChange({ kind: "theme", themeId })
    }
    themeLink.addEventListener("load", notify)
    themeLink.addEventListener("error", notify)
  ...
  _applyAllSettings() {
    ...
    if (sidebar) {
      ...
      // The resize handle is created imperatively by the core module's init()
      // (queried off `.pc-layout__sidebar--resizable`) and is visible purely by
      // existing in the DOM — CSS does NOT gate it on the class. So toggling the
      // class alone is inert: enabling needs an explicit init() to build the
      // handle, and disabling needs the handle removed or it lingers, live and
      // draggable. init() at DOMContentLoaded already ran and bailed (no
      // resizable sidebar then), so this runtime re-init is what actually wires
      // drag-to-resize on.
      const resize = window.pureAdmin && window.pureAdmin.components && window.pureAdmin.components.sidebarResize
      if (this._getSetting("sidebar-resizable") === "true") {
        sidebar.classList.add("pc-layout__sidebar--resizable")
        if (resize && typeof resize.init === "function") resize.init()
      } else {
        sidebar.classList.remove("pc-layout__sidebar--resizable")
        // Drop the handle so it isn't left visible/draggable. Removing the node
        // (not just the class) also resets the module: its next init() sees no
        // existing handle and rebinds fresh listeners. Width override is left
        // intact — a deliberately-sized sidebar shouldn't jump on toggle-off.
        const handle = sidebar.querySelector(".pc-sidebar-resize")
        if (handle) handle.remove()
      }
    }

    // Search position (rc12 + rc15) — reveal ONE of the five search entry points
    // and hide the rest. Targeted by class (library-general): no-ops in a consumer
    // that renders none of them. The inline field takes the centre slot, so it
    // swaps out the page title. Showing/hiding a header slot changes the row's
    // content, so re-run navbar-fit to re-settle priorities.
    const searchPos = this._getSetting("search-position")
    const inlineSearch = document.querySelector(".pc-navbar-search--field")
    const compactSearch = document.querySelector(".pc-navbar-search--sm")
    const navbarInput = document.querySelector(".pc-navbar-search--input")
    // The sidebar trigger and the type-and-go form share `.pc-sidebar__search`;
    // disambiguate on the `--input` modifier so each toggles independently.
    const sidebarSearch = document.querySelector(".pc-sidebar__search:not(.pc-sidebar__search--input)")
    const sidebarInput = document.querySelector(".pc-sidebar__search--input")
    const headerTitle = document.querySelector(".pc-page-header")
    if (inlineSearch) inlineSearch.hidden = searchPos !== "navbar-inline"
    if (compactSearch) compactSearch.hidden = searchPos !== "navbar-compact"
    if (navbarInput) navbarInput.hidden = searchPos !== "navbar-input"
    if (sidebarSearch) sidebarSearch.hidden = searchPos !== "sidebar"
    if (sidebarInput) sidebarInput.hidden = searchPos !== "sidebar-input"
    if (headerTitle) headerTitle.hidden = searchPos === "navbar-inline"
    if (searchPos) body.dataset.searchPosition = searchPos
    else delete body.dataset.searchPosition
    const navFit = window.pureAdmin && window.pureAdmin.components && window.pureAdmin.components.navFit
    if (navFit && typeof navFit.relayoutAll === "function") navFit.relayoutAll()

    // Command palette size (rc15) — set the --sm/--lg/--xl width preset on the
    // palette. Targeted by class (library-general): no-ops when no palette is
    // rendered on the page. The palette opens via a client-side
    // `pa:command-palette:open` dispatch (classList toggle of --active), so this
    // size class survives open/close. Allowlist-checked against poisoned storage.
    const paletteSize = this._getSetting("command-palette-size")
    document.querySelectorAll(".pa-command-palette").forEach((palette) => {
      palette.classList.remove(...CMD_PALETTE_SIZES.map((s) => `pa-command-palette--${s}`))
      if (CMD_PALETTE_SIZES.includes(paletteSize)) {
        palette.classList.add(`pa-command-palette--${paletteSize}`)
      }
    })
    ...
    if (this._getSetting("sidebar-mode") === "sticky") {
      body.classList.add("pc-layout--sticky")
    } else {
      body.classList.remove("pc-layout--sticky")
    }
