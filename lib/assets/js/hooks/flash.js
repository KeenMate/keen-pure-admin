...
export const PureAdminFlash = {
  ...
  _showFlash(flash) {
    ...
    if (actions.length > 0) {
      contentHtml += `<div class="pa-alert__actions">`
      for (const action of actions) {
        const btnVariant = action.variant || 'secondary'
        contentHtml += `<button class="pa-btn pa-btn--sm pa-btn--${btnVariant}" data-action="${this._escapeAttr(JSON.stringify(action))}">${this._escapeHtml(action.label || 'Action')}</button>`
      }
      contentHtml += `</div>`
    }
