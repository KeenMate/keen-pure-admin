 ...
 *                                             container edge)
 *
 * --- Legacy 2-pane root shorthand (keen-pure-admin extension) ---
 *   Not part of upstream. `normalizeLegacyMarkup()` translates the old
 *   root-level 2-pane shorthand into per-pane attributes before init so a
 *   single code path handles both forms:
 *     data-pa-splitter-min-start="200px" | "20%"  → start pane -min
 *     data-pa-splitter-max-start="60%"  | "800px" → start pane -max
 *     data-pa-splitter-default="280px"  | "30%"   → start pane -size
...
(function () {
    ...
    // keen-pure-admin extension (not upstream). Translate the legacy 2-pane
    // shorthand into per-pane attributes so the single N-pane code path
    // handles both forms. The old root-level -min-start / -max-start /
    // -default move onto the start pane; -minimize="start|end" becomes the
    // marker attribute on the named pane. Existing per-pane attributes win
    // (consumer-set values not overwritten), so mixed-style markup still
