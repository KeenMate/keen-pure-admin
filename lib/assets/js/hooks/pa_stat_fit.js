 ...
 *
 * The core IIFE exposes `window.PaStatFit = { init, refresh }` and auto-runs
...
export const PureAdminStatFit = {
  mounted() {
    if (window.PaStatFit && typeof window.PaStatFit.init === "function") {
      window.PaStatFit.init(this.el)
    }
  },
  ...
  updated() {
    ...
    if (window.PaStatFit && typeof window.PaStatFit.refresh === "function") {
      window.PaStatFit.refresh(this.el)
    }
  }
}
