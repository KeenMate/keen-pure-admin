 ...
 *
 * The core IIFE exposes `window.PaOverflow = { init, initAll }` and auto-runs
 ...
 *
 * `destroyed()` calls the keen-added `window.PaOverflow.destroy(this.el)` so
...
function initOverflow(el) {
  if (!window.PaOverflow) return
  if (typeof window.PaOverflow.init === "function") {
    window.PaOverflow.init(el)
  } else if (typeof window.PaOverflow.initAll === "function") {
    window.PaOverflow.initAll(el)
  }
}
...
export const PureAdminOverflow = {
  ...
  destroyed() {
    if (window.PaOverflow && typeof window.PaOverflow.destroy === "function") {
      window.PaOverflow.destroy(this.el)
    }
  }
}
