 ...
 *
 * ONE KEEN ADDITION (everything else is byte-for-byte upstream, including the
 * `.pa-overflow, .pa-card__actions--overflow` auto-init and the
 * `window.PaCardActionsOverflow` back-compat alias): a `destroy(root)`
 * teardown fn, also exposed as `window.PaOverflow.destroy`. Upstream targets
 * classic server-rendered pages where a toolbar is never removed, so its init
 * never tears down — the ResizeObserver / MutationObserver and the
 * body-portal "more" menu just live for the page lifetime. Under LiveView an
 * element can be removed by a diff at any time, so the hooks call `destroy()`
 * in `destroyed()` to disconnect the observers, restore the pulled-out items
 * to the wrapper, and remove the orphaned body menu + trigger. Same rationale
 * as the `normalizeLegacyMarkup()` shim kept in `splitter_core.js`. To support
 * it, the per-instance handles (`menu` / `trigger` / observers / `closeMenu` /
 * `moveToRoot` / `ordered`) are stashed on `root[STATE_FLAG]` at the end of
 * `init()`; nothing else in the algorithm changes.
 * ---------------------------------------------------------------------------
