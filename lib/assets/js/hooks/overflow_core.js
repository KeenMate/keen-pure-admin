/**
 * VERBATIM PORT of @keenmate/pure-admin-core `src/js/overflow.js`
 * (v2.9.0-rc06). Re-copy from upstream when it changes.
 * Wrapped by the thin Phoenix hooks in `overflow.js` (`.pa-overflow`) and
 * `card_actions_overflow.js` (`.pa-card__actions--overflow`).
 *
 * TWO KEEN ADDITIONS (everything else is byte-for-byte upstream, including the
 * `.pa-overflow, .pa-card__actions--overflow` auto-init and the
 * `window.PaCardActionsOverflow` back-compat alias):
 *
 * (1) a `destroy(root)` teardown fn, also exposed as `window.PaOverflow.destroy`.
 * Upstream targets classic server-rendered pages where a toolbar is never
 * removed, so its init never tears down — the ResizeObserver / MutationObserver
 * and the body-portal "more" menu just live for the page lifetime. Under
 * LiveView an element can be removed by a diff at any time, so the hooks call
 * `destroy()` in `destroyed()` to disconnect the observers, restore the
 * pulled-out items to the wrapper, and remove the orphaned body menu + trigger.
 * Same rationale as the `normalizeLegacyMarkup()` shim kept in
 * `splitter_core.js`. To support it, the per-instance handles (`menu` /
 * `trigger` / observers / `closeMenu` / `moveToRoot` / `ordered`) are stashed on
 * `root[STATE_FLAG]` at the end of `init()`; nothing else in the algorithm
 * changes.
 *
 * (2) a one-line guard in `moveToRoot()`: re-append the trigger if it isn't a
 * child of `root` before `insertBefore(el, trigger)`. Upstream assumes the
 * JS-appended trigger stays put; under LiveView a DOM patch can delete it (it
 * isn't in the server-rendered markup), and the next relayout then throws
 * `NotFoundError: ... not a child of this node`. The `<.overflow>` /
 * `<.card actions_variant="overflow">` components set `phx-update="ignore"` to
 * stop the patch from touching these children at all — this guard is the
 * belt-and-suspenders so the raw class survives even without it.
 * ---------------------------------------------------------------------------
 ...
 * (shipped by @keenmate/pure-admin-core) supplies that for `.pa-overflow`;
 * the card variant carries equivalent rules in `_cards.scss`.
 *
 ...
 *   destroy(el)    — [keen addition] tear down a single element (see header)
 *
...
(function () {
    ...
    // [keen addition] where init() stashes per-instance handles for destroy().
    var STATE_FLAG = '__paOverflowState';
    ...
    function init(root) {
        ...
        function moveToRoot(el) {
            ...
            // [keen addition] A LiveView DOM patch can delete the JS-appended
            // trigger (it isn't in the server markup), leaving it detached from
            // root; insertBefore(el, trigger) would then throw NotFoundError.
            // Re-attach the trigger as last child before we anchor against it.
            if (trigger.parentNode !== root) root.appendChild(trigger);
        ...
        // [keen addition] observers hoisted to init scope so destroy() can
        // disconnect them (upstream declares them inline and never tears down).
        var ro = null;
        var mo = null;
        ...
        // [keen addition] stash handles so destroy(root) can tear this down.
        root[STATE_FLAG] = {
            trigger: trigger,
            menu: menu,
            ordered: ordered,
    ...
    // [keen addition] tear down a single element (see file header). Idempotent
    // and safe to call on an element that was never init'd. Restores the
    // pulled-out items to the wrapper so LiveView reclaims them with the
    // element it's removing, then drops the body-portal menu + trigger.
    function destroy(root) {
        if (!root) return;
        var state = root[STATE_FLAG];
        if (state) {
            state.closeMenu();
            if (state.ro) state.ro.disconnect();
            if (state.mo) state.mo.disconnect();
            state.ordered.slice().sort(function (a, b) {
                return a.domIndex - b.domIndex;
            }).forEach(function (item) {
                state.moveToRoot(item.el);
            });
    ...
    var api = { init: init, initAll: initAll, destroy: destroy };
    ...
    // the same { init, initAll, destroy } pair under a second name so consumers
    // that re-init card-actions after injecting dynamic markup keep a handle.
