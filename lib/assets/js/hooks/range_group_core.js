/**
 * VERBATIM PORT of `@keenmate/pure-admin-core` `src/js/range-group.js`.
 *
 * Do not hand-edit — re-sync from upstream on version bumps. Exposes
 * `window.PaRangeGroup = { init }` where `init(scope?)` builds every
 * `[data-range-group]` under `scope` (default: document); building is
 * idempotent per root (guarded by `root._rangeGroup`). Wrapped by the thin
 * Phoenix hook in `range_group.js`.
 */
