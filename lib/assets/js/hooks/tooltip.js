...
// Copy the trigger's tooltip variant/modifier classes (semantic --primary…,
// numeric --color-1..9, --multiline, …) onto the generated floating element,
// skipping the placement classes. Both the hook path and the delegated listener
// build the floating tooltip via createFloatingTooltip, so doing it here keeps
// variants working for both (the hook path previously copied none).
function copyTooltipVariants(trigger, floatingEl) {
...
function createFloatingTooltip(trigger, text, position) {
  ...
  copyTooltipVariants(trigger, el)
  el.textContent = text
...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('mouseover', function (e) {
    ...
    // Variant/modifier classes are copied inside createFloatingTooltip.
  })
