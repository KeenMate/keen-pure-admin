 ...
 *
 * Mounted on `.kpi-tile` elements that have a `.kpi-tile__detail` child.
 * On mount, moves the detail element to <body> so ancestor overflow:hidden
 * / transform / clip-path can't clip it. On mouseenter/mousemove updates
 * the popover position via Floating UI anchored to a virtual reference
 * element at the cursor coordinates.
 *
 ...
 * - `pointer-events: none` (set in CSS on `.kpi-tile__detail`) so the
 *   cursor passes through and tile mouseleave fires reliably.
...
export const PureAdminKpiTile = {
  mounted() {
    this.detail = this.el.querySelector('.kpi-tile__detail')
