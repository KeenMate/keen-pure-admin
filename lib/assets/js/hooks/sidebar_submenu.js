...
const STORAGE_KEY = "pc-sidebar-submenus"
...
export const PureAdminSidebarSubmenu = {
  mounted() {
    const li = this.el
    const submenuId = li.querySelector(".pc-sidebar__submenu")?.id
    if (!submenuId) return
    ...
    if (!hasActivePage) {
      ...
      if (submenuId in stored) {
        ...
        li.classList.toggle("pc-sidebar__item--open", isOpen)
        const submenu = li.querySelector(".pc-sidebar__submenu")
        if (submenu) {
          submenu.classList.toggle("pc-sidebar__submenu--open", isOpen)
        }
      }
    }
    ...
    const observer = new MutationObserver(() => {
      const isOpen = li.classList.contains("pc-sidebar__item--open")
      const stored = getStoredState()
