 ...
 *   resize the panel.
 * - The width is written to the `--pc-local-detail-panel-width` CSS
 *   custom property on `<html>`, so any panel reading
 *   `width: var(--pc-local-detail-panel-width)` follows the change.
...
function startDrag(event, panel, handle) {
  ...
  const onMove = (moveEvent) => {
    ...
    document.documentElement.style.setProperty("--pc-local-detail-panel-width", `${next}px`)
  }
