...
pure-admin-core, the grid + mode classes (`pa-row`/`pa-col*`/`pa-mode-*` → `pc-*`) and
the entire runtime CSS-variable surface (`--pa-*` → `--pc-*`) are de-branded. Emitted
component markup, JS hooks that read/write custom properties, and vendored theme/core
CSS are updated. Apps migrate any own markup/overrides via find/replace (`--pa-` →
`--pc-`, and the `pa-row`/`pa-col`/`pa-mode-` classes). `--base-*` is unchanged. Requires
rc04-built theme sources reinstalled via `npx @keenmate/pureadmin themes install`.
