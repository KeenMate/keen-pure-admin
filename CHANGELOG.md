...
- **`PureAdminProfilePanel` — URL scheme check before navigation (audit finding #6).** Favourite items previously assigned `dataset.href` directly to `window.location.href`. Now only `http:` / `https:` / `mailto:` / `tel:` absolute URLs and same-origin relative paths are followed; `javascript:`, `data:`, `vbscript:`, and `file:` URIs are rejected.
