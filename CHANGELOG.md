...
### Pure-admin 2.9.0 sync (in progress)

First pass — small markup / token alignments. Larger 2.9.0 surface (`<.splitter>`, card-actions overflow / responsive) still to come; peer-dep stays at `^2.8.0` until that lands.
...
- **`PureAdminCardActionsOverflow` hook** — port of `card-actions-overflow.js` (~200 LOC) into Phoenix LiveView lifecycle. `mounted()` sets up the trigger button (body-portal menu reuses `pa-btn-split__menu` for styling), ResizeObserver on the wrapper + parent, MutationObserver on the `data-pa-actions-overflow-from` attr, and FloatingUI positioning (falls back to hand-rolled `getBoundingClientRect` if FloatingUI isn't loaded — matches `<.btn_split>`'s approach). `destroyed()` tears everything down — disconnects observers, restores original button classes (the hook swaps them to `pa-btn-split__item` while items live in the menu), removes the body-portal menu and trigger.
